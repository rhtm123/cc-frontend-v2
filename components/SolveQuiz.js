...
{!optionLoading && <div>
  {options.map((option,key)=>
    ...
    <div > {currentCount>0 && <button data-aos="fade" className="btn btn-primary" onClick={()=>handleNP('p')}>Previous</button>}</div>

<div> 
          {currentCount<questions.length-1 ? <button data-aos="fade" className="btn btn-primary" onClick={()=>handleNP('n')}>Next</button>: <div>
              
              <button data-aos="fade" className="btn btn-primary" onClick={()=>handleSubmit()}>See the Result</button>
              </div>
  ...
  <br />
    ...
    <button onClick={()=>startNow()} data-aos="fade" className="btn btn-primary">
          Start Quiz
    </button>

...
function QuizzesPage() {
  ...
  const getquizzess = () => {
    let url =
      process.env.API_URL +
      `quiz/quiz_list/?ordering=-created&creator=&is_published=true&allowed_to=0`;
    fetch(url)
      .then(async (response) => {
        if (response.ok) {
          let data1 = await response.json();
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="MCQ questions, Quizzes, Quiz on Python, HTML, CSS, JavaScript, React, Django, Database, SQL"
        />
      </Head>
